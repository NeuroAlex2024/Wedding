...
- Каталог `src/shared` содержит структуры данных, которые используются одновременно клиентом и сервером.
- Темы приглашений и связанные значения (цвета, шрифты, дефолтные подписи) правятся только в `src/shared/themes.json`. Клиентское приложение и сервер Express читают этот файл напрямую, поэтому изменений в других файлах не требуется.
